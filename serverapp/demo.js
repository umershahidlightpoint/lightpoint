"use strict";
const { Fund, Account, Customer, Ledger, AccountType } = require("./src/models");
...
const accountTypes = [
  {
    name: "Liabilities",
    notes: "",
    accounts: ["LB-Account1", "LB-Account2", "LB-Account3", "LB-Account4", "LB-Account5"]
  },
  {
    name: "Expenses",
    notes: "",
    accounts: ["EX-Account1", "EX-Account2", "EX-Account3", "EX-Account4", "EX-Account5"]
  },
  {
    name: "Equity",
    notes: "",
    accounts: ["EQ-Account1", "EQ-Account2", "EQ-Account3", "EQ-Account4", "EQ-Account5"]
  },
  {
    name: "Asset",
    notes: "",
    accounts: ["AT-Account1", "AT-Account2", "AT-Account3", "AT-Account4", "AT-Account5"]
  },
  {
    name: "Revenue",
    accounts: ["REV-Account1", "REV-Account2", "REV-Account3", "REV-Account4", "REV-Account5"]
  }
];
const createAccountTypes = async(accountTypes) => {
  return await accountTypes.map(async(accountType) => {
    let accountTypeRecord = await AccountType.findOne({where: {'name': accountType.name}});
    ...
    const accounts = await accountType.accounts.map(async(account) => {
      let accountRecord = await Account.findOne({where: {'name': account, 'account_type_id' : accountTypeRecord.id}});
...
(async () => {
  ...
  const savedFunds = importFunds(fundNames);
  const customers = importCustomers(2);
